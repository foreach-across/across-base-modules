...
== Creating Auditing tables with AuditableSchemaInstaller

When you with to create Auditable tables with Across and {module-name}, you can create an installer that extends `AuditableSchemaInstaller`.
This installer will decorate your tables with the required columns to support Auditable entities.

First of all, the entity itself should extend `SettableIdAuditableEntity` as seen below.

.A User entity ready to be used as an Auditable object.
[source,java,indent=0]
[subs="verbatim,quotes,attributes"]
----

@Entity
@Table(name = DomainManagerConstants.TABLE_CUSTOMERS)
...
Secondly, create an installer that will add the auditable columns to your entities.
The installer should return a list of all tables that need to be processed.

.An AuditableSchemaInstaller .
[source,java,indent=0]
[subs="verbatim,quotes,attributes"]
----
@Order(2)
@Installer(description = "Adds auditable columns to all entities", version = 1)
public class MyAuditableInstaller extends AuditableSchemaInstaller
{
	@Override
	protected Collection<String> getTableNames() {
		return Collections.singleton( DomainManagerConstants.TABLE_CUSTOMERS );
	}
}
----

Note the use of `@Order(2)` which might be necessary to ensure that your `AuditableSchemaInstaller` is run *after* running your `CoreSchemaInstaller`.
