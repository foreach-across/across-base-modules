...
== Index patterns
Index-patterns used by Kibana can be autocreated at startup.
The desired patterns can be configured in logging-module-test-web/docker/index-patterns/create-default-patterns.sh[create-default-patterns.sh]

[options="header", cols="<2,<2"]
|===============
|Key|Description
|application_name|Name of the application. This should be the same as the 'logging.logstash.application' property
|environments|Environments for which an index-pattern should be created
|appenders|Appenders for which an index-pattern should be created
|default_index_pattern_environment|Environment for which an default index-pattern should be created.
|default_index_pattern_appender|Appender for which an default index-pattern should be created.
|===============

Combination of default_index_pattern_environment and default_index_pattern_appender will define the default index-pattern.
